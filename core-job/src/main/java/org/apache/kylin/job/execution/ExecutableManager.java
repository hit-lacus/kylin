...
public class ExecutableManager {
    ...
    public String getSampleDataFromHDFS(String resPath, final int nLines) {
        try {
            ...
            try (FSDataInputStream din = fs.open(path);
                ...
                int offset = sampleData.toString().getBytes(StandardCharsets.UTF_8).length + 1;
    ...
    public void updateJobOutput(String project, String jobId, ExecutableState newStatus, Map<String, String> info, String output, String logPath) {
        ...
        if (project != null) {
            // Write output to HDFS for spark driver side log content
            logger.debug("Update Job Output To HDFS for {}", jobId);
            updateJobOutputToHDFS(project, jobId, output, logPath);
        }
    }

    private void updateJobOutputToHDFS(String project, String jobId, String output, String logPath) {
    ...
    private void updateJobOutputToHDFS(String resPath, ExecutableOutputPO obj) {
